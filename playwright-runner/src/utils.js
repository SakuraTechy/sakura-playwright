...
export function parseArgs(argv = process.argv.slice(2), env = process.env) {
  ...
  const config = {
    ...
    slowMoMs: parseNonNegativeInt(args['slow-mo'] || mergedEnv.RUNNER_SLOW_MO_MS, 0),
    finishDelayMs: parseNonNegativeInt(args['finish-delay'] || mergedEnv.RUNNER_FINISH_DELAY_MS, 0),
    timeoutMs: parsePositiveInt(args.timeout || mergedEnv.RUNNER_STEP_TIMEOUT_MS, 6000),
    caseTimeoutMs: parsePositiveInt(args['case-timeout'] || mergedEnv.RUNNER_CASE_TIMEOUT_MS, 600000),
...
export async function withTimeout(promise, timeoutMs, label) {
  ...
  const timeout = new Promise((_, reject) => {
    timer = setTimeout(() => {
      reject(new RunnerError('ACTION_TIMEOUT', `${label || 'Operation'} timed out after ${timeoutMs}ms`));
    }, timeoutMs);
  });
  try {
