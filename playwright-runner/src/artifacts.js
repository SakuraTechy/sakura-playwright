...
import { timestampForPath } from './utils.js';

export async function createRunArtifacts({ artifactDir, caseId }) {
  const runId = `${caseId}-${timestampForPath()}`;
  const runDir = path.resolve(process.cwd(), artifactDir, 'runs', runId);
